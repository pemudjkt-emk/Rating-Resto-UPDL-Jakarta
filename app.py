...
st.set_page_config(page_title="Rating Resto UPDL", page_icon="🍽️", layout="wide")
...
div[data-testid="stFeedback"] {
    transform: scale(8.25); /* Ukuran bintang ekstra besar sesuai request */
...
<h1>RATING KEPUASAN PELANGGAN<br>RESTO UPDL JAKARTA</h1>
</div>
...
<img class="logo-pln" src="data:image/png;base64,{img_pln}" alt="Logo PLN" onerror="this.style.display='none'">
    ...
    with st.container(border=True):
        ...
        with col1_kiri:
            st.markdown("<p class='tanya-teks'>BAGAIMANA RASA DAN KESEGARAN MAKANAN?</p>", unsafe_allow_html=True)
        with col1_kanan:
            rasa = st.feedback("stars", key=f"bintang_rasa_{st.session_state.sesi_id}")
        ...
        with col3_kiri:
            st.markdown("<p class='tanya-teks'>BAGAIMANA VARIASI MENU KAMI?</p>", unsafe_allow_html=True)
        with col3_kanan:
            variasi = st.feedback("stars", key=f"bintang_variasi_{st.session_state.sesi_id}")
...
# ==========================================
    ...
    try:
        ...
        data_baru = pd.DataFrame([{
            ...
            "Variasi Menu": st.session_state.data_temp["variasi"] + 1 
        }])
